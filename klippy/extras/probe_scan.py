...
class ProbeScanHelper:
    ...
    def _flush_events(self):
        ...
        for params in raw_samples:
            ...
            for i in range(count):
                ...
                logging.debug("probe_scan: event state=%d print_time=%.4f "
    ...
    def start_collection(self, print_time, poll_us):
        ...
        logging.debug("probe_scan: start_collection oid=%d clock=%d "
...
class ProbeScanController:
    ...
    def run_scan(self, printer, probe_helper, waypoints, start_x, start_y,
        ...
        logging.debug("probe_scan: static zig-zag scan z_top=%.3f "
        ...
        logging.debug("probe_scan: queued %d segments, waiting for "
        ...
        logging.debug("probe_scan: scan complete, %d segments queued, "
...
class PrinterProbeScan:
    ...
    def _handle_connect(self):
        if self._cal_point is None:
            if self._printer.lookup_object('bed_mesh', None) is None:
                raise self._printer.config_error(
                    "probe_scan: calibration_point must be configured "
                    "when [bed_mesh] is not present")
    ...
    def _measure_hysteresis(self, gcmd):
        ...
        logging.debug("probe_scan: hysteresis events: %d correlated, "
    ...
    def perform_scan_mesh(self, gcmd, base_points, x_count, y_count,
        ...
        if log_file:
            scan_dir = os.path.dirname(log_file)
        else:
        ...
        try:
            ...
            logging.error("probe_scan: Could not write scan data to %s"
                          % scan_path)
        ...
        if generate_mesh:
            ...
            if zero_ref_pos is not None:
                try:
                    ...
                    raise gcmd.error(
                        "probe_scan: No probe data near "
                        "zero_reference_position (%.1f, %.1f). "
