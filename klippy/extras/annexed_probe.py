...
class AnnexedProbe:
    ...
    def _align_z(self):
        ...
        curtime = self.printer.get_reactor().monotonic()
        if not self.homed:
            self.homed = self.toolhead.get_status(curtime)['homed_axes']
        if self.dock_fixed_z:
            ...
            if 'z' in self.homed:
                ...
                raise self.printer.command_error(
                    "Unable to attach probe, must home Z axis first")
    def _force_z_hop(self):
        self.debugOutput('_force_z_hop')
        if not self.z_hopped:
            self.debugOutput('_force_z_hop: hopping z')
            tpos = self.toolhead.get_position()
            ...
            self.z_hopped = True
    def _check_probe_distance(self):
        self.debugOutput('_check_probe_distance')
        tpos = self.toolhead.get_position()
        dock = self.dock_position
        if self.dock_safe_distance > sqrt((tpos[0]-dock[0])**2 +
                                          (tpos[1]-dock[1])**2 ):
            return True
        else:
            return False
    ...
    def _handle_homing_rails_begin(self, homing_state, rails):
        ...
        if self.get_probe_status() == PROBE_ATTACHED:
            ...
            if (any(a in ['x','y'] for a in homing_axes)
              and not 'z' in self.homed):
                self.debugOutput('_handle_homing_rails_begin: z not homed, hopping z')
                self._force_z_hop()
            if ('z' in homing_axes and not self.homing_requested
            and 'xy' in self.homed and not self.z_homes_positive):
                self.detach_probe(self.toolhead.get_position())
            if 'z' in homing_axes and self.homing_requested:
