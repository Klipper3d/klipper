...
class ExcludeObject:
    def __init__(self, config):
        ...
        self.last_position_extruded = [0., 0., 0., 0., 0.]
        self.last_position_excluded = [0., 0., 0., 0., 0.]
    ...
    def _normal_move(self, newpos, speed):
        ...
        self.last_position_extruded[:] = self.last_position
        self.max_position_extruded = max(self.max_position_extruded, newpos[4])
        ...
        if self.extruder_adj != 0 and \
            newpos[4] != self.last_position_excluded[4]:
            offset[4] += self.extruder_adj
            self.extruder_adj = 0
    ...
    def _ignore_move(self, newpos, speed):
        ...
        offset[4] = offset[4] + newpos[4] - self.last_position[4]
        self.last_position[:] = newpos
        self.last_position_excluded[:] =self.last_position
        self.max_position_excluded = max(self.max_position_excluded, newpos[4])
    ...
    def _move_from_excluded_region(self, newpos, speed):
        ...
        self.extruder_adj = self.max_position_excluded \
            - self.last_position_excluded[4] \
            - (self.max_position_extruded - self.last_position_extruded[4])
