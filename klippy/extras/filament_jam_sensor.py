...
#
# Copyright (C) 2019  Eric Callahan <arksine.code@gmail.com> - Windo.AC
#
...
class JamSensor:
    def __init__(self, config):
        ...
        self.slow_usage = config.getfloat('slow_usage', 0. ) / 100.
        self.slow_speed = config.getfloat('slow_speed', 10., above=0.) / ( 60. * 100. )
        self.slow_gcode = config.get('slow_gcode', None)
        self.jam_usage = config.getfloat('jam_usage', 400. , above=100.) / 100.
        self.jam_gcode = config.get('jam_gcode', None)
        self.jam_pause = config.getboolean('jam_pause', True)
        self.slow_speed_resume = config.getfloat('slow_speed_resume', 1. , above=0.) / ( 60. * 100. ) # Zore mean not resume
        ...
        self.slowest_speed = 0.5 / 60.
        self.full_speed = 1. / 60.
        self.cmd_SET_FILAMENT_JAM_SENSOR_help = "Set param of FILAMENT_JAM_SENSOR"
        self.gcode.register_mux_command(
            "SET_FILAMENT_JAM_SENSOR", "SENSOR", self.name,
    ...
    def _handle_ready(self):
        ...
        if self.base_usage is None :
            ...
            self.enable = False
        elif self.slow_usage > 0. and self.slow_usage >= self.jam_usage :
    ...
    def _signal_handler(self, eventtime, state):
        if self.enable and state == 1 :
            ...
            if delta_usage > 0 and self.check_jam(eventtime, delta_usage) and self.slow_usage > 0. and self.gcode.speed_factor < self.full_speed: # won't run check_jam if delta_usage<0
                ...
                self.gcode.respond_info("speed_factor:%d" % ( self.gcode.speed_factor * 60. * 100. ))
            self.slow_triggered = False
            self.jam_triggered = False
    def _timer_handler(self, eventtime):
        if self.enable :
            ...
            if delta_usage > 0 and self.timer_usage_last != new_usage : # ignore when it's not move # won't run check_jam if delta_usage<0
    ...
    def check_jam(self, eventtime, delta_usage):
        ...
        if temp_rate > self.jam_usage :
            #detect jam
            #self.filament_usage_last = self.get_filament_usage() # restart the last value avoid trigger when pause
        ...
        elif self.slow_usage > 0. and temp_rate > self.slow_usage :
            #detect slow
    ...
    def cmd_SET_FILAMENT_JAM_SENSOR(self, params):
        self.timer = self.gcode.get_float('TIMER', params, self.timer, minval=0.)
        self.base_usage = self.gcode.get_float('BASE_USAGE', params, self.base_usage, minval=0.)
        self.slow_usage = self.gcode.get_float('SLOW_USAGE', params, self.slow_usage * 100. ) / 100.
        temp_jam_usage = self.gcode.get_float('JAM_USAGE', params, self.jam_usage * 100. , minval=200.) / 100.
        if temp_jam_usage is not None and temp_jam_usage > self.slow_usage : 
            self.jam_usage = temp_jam_usage
        self.slow_speed_resume = self.gcode.get_float('SLOW_SPEED_RESUME', params, self.slow_speed_resume * 100. * 60. , minval=0. ) / ( 60. * 100. )
        self.slow_speed = self.gcode.get_float('SLOW_SPEED', params, self.slow_speed * 100. * 60. , minval=0. ) / ( 60. * 100. )
        ...
        self.gcode.respond_info("%s(%s): timer = %.2f | base_usage = %.2f | slow_usage = %.2f | jam_usage = %.2f | slow_speed = %.2f | slow_speed_resume = %.2f | enable = %s | debug = %s | action = %s" % ( self.mname , self.name , self.timer,self.base_usage,self.slow_usage*100.,self.jam_usage*100.,self.slow_speed*100.*60.,self.slow_speed_resume*100.*60.,self.enable,self.debug,self.action )) 
        # reset some counter
