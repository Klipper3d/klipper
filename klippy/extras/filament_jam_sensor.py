...
class JamSensor:
    def __init__(self, config):
        ...
        self.slow_usage = config.getfloat('slow_usage', None, above=self.base_usage)
        self.slow_speed = config.getfloat('slow_speed', 60., above=0.)
        self.slow_gcode = config.get('slow_gcode', None)
        self.jam_usage = config.getfloat('jam_usage', None, above=self.slow_usage)
    ...
    def _handle_ready(self):
        ...
        if self.base_usage is None or self.slow_usage is None or self.jam_usage is None :
            logging.exception("base_usage / slow_usage / jam_usage must defined")
            self.enable = False
        elif self.base_usage >= self.slow_usage or self.slow_usage >= self.jam_usage :
    ...
    def check_jam(self, eventtime, delta_usage):
        if delta_usage > self.jam_usage :
            ...
            if self.action and self.pause_resume.captured_position is None : # only trigger pause when not in pause status
    ...
    def cmd_SET_FILAMENT_JAM_SENSOR(self, params):
        ...
        temp_slow_usage = self.gcode.get_float('SLOW_USAGE', params, self.slow_usage, minval=0.)
        if temp_slow_usage is not None and temp_slow_usage > self.base_usage : 
            self.slow_usage = temp_slow_usage
        temp_jam_usage = self.gcode.get_float('JAM_USAGE', params, self.jam_usage, minval=0.)
        ...
        self.gcode.respond_info("%s(%s): timer = %.2f | base_usage = %.2f | slow_usage = %.2f | jam_usage = %.2f | enable = %s | debug = %s | action = %s" % ( self.mname , self.name , self.timer,self.base_usage,self.slow_usage,self.jam_usage,self.enable,self.debug,self.action )) 
        # reset some counter
