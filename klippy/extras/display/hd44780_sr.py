# Support for HD44780_SR (20x4 text) LCD displays
#
...
class HD44780_SR:
    def __init__(self, config):
        ...
        for pin_params in pins:
            if mcu is not None and pin_params['chip'] != mcu:
                raise ppins.error("hd44780_sr all pins must be on same mcu")
            mcu = pin_params['chip']
    ...
    def build_config(self):
        self.mcu.add_config_cmd(
            "config_hd44780_sr oid=%d rs_pin=%s e_pin=%s"
            " d4_pin=%s d5_pin=%s d6_pin=%s d7_pin=%s delay_ticks=%d" % (
                ...
                self.mcu.seconds_to_clock(HD44780_SR_DELAY)))
    ...
    def send(self, cmds, is_data=False):
        ...
        #logging.debug("hd44780_sr %d %s", is_data, repr(cmds))
    def flush(self):
    ...
    def init(self):
        ...
        # Add custom fonts
        self.glyph_framebuffer[:len(HD44780_SR_chars)] = HD44780_SR_chars
