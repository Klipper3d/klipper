...
# Copiright (C) 2021  Olivier Bossi <olivier.bossi@protonmail.ch>
#
...
class hd44780_i2c:
    def __init__(self, config):
        ...
        self.hd44780_protocol_init = config.getboolean('hd44780_protocol_init',
                                                       True)
        self.backlight = config.getboolean('backlight',
                       True)
        # spi config
        ...
        self.data_mask = (1<<0)
        self.command_mask = (0<<0)
        self.enable_mask = (1<<2)
        self.backlight_mask = (1<<3)

        self.set_backlight_mask(self.backlight)
    ...
    def set_backlight_mask(self,value):
        if value:
            self.backlight_mask = (1<<3)
        else:
            self.backlight_mask = (0<<3)

    cmd_HD_IIC_LCD_BACKLIGHT_help = "Set I2C HD44780 backlight value"
    def cmd_HD_IIC_LCD_BACKLIGHT(self, gcmd):
        value = gcmd.get_int('VALUE', 1, minval=0, maxval=1)
        self.set_backlight_mask(value==1)
        self.i2c.i2c_write([self.backlight_mask])
