...
class hd44780_i2c:
    ...
    def _encode_nibble(self, val, is_data):
        mask = self.data_mask if is_data else self.command_mask
        mask |= self.backlight_mask
        nibble = val & 0xF0
        return [
            nibble | mask,
            nibble | mask | self.enable_mask,
            nibble | mask
        ]

    def _encode_byte(self, val, is_data):
        high = val & 0xF0
        low = (val << 4) & 0xF0
        return (self._encode_nibble(high, is_data)
                + self._encode_nibble(low, is_data))
    ...
    def send_data(self, data_bytes):
        ...
        for offset in range(0, len(data_bytes), max_chunk):
            ...
            packet = bytearray()
            for b in sub_data:
                packet.extend(self._encode_byte(b, is_data=True))
            try:
    ...
    def send_4_bits(self, cmd, is_data, minclock=0):
        data = self._encode_nibble(cmd & 0xF0, is_data)
        try:
    ...
    def send(self, cmds, is_data=False):
        ...
        packet = bytearray()
        for cmd in cmds:
            packet.extend(self._encode_byte(cmd, is_data))
        try:
            self.i2c.i2c_write_noack(bytes(packet))
        except Exception as e:
            logging.error("LCD I2C Write Error: %s" % (str(e),))
