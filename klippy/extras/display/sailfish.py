# Support for HD44780 / Sailfish (20x4 text) LCD displays
#
...
class Sailfish:
    def __init__(self, config):
        ...
        for pin_params in pins:
            if mcu is not None and pin_params['chip'] != mcu:
                raise ppins.error("sailfish_lcd all pins must be on same mcu")
            mcu = pin_params['chip']
    ...
    def build_config(self):
        self.mcu.add_config_cmd(
            "config_sailfish_lcd oid=%d strobe_pin=%s data_pin=%s"
        ...
        self.send_cmds_cmd = self.mcu.lookup_command(
            "sailfish_lcd_send_cmds oid=%c cmds=%*s", cq=cmd_queue)
        self.send_data_cmd = self.mcu.lookup_command(
            "sailfish_lcd_send_data oid=%c data=%*s", cq=cmd_queue)
    def send(self, cmds, is_data=False):
        ...
        #logging.debug("sailfish_lcd %d %s", is_data, repr(cmds))
    def flush(self):
