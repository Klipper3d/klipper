...
class Heater:
    def __init__(self, config, sensor):
        ...
        # Get config options
        self._min_temp = config.getfloat('min_temp', minval=KELVIN_TO_CELSIUS)
        self._max_temp = config.getfloat('max_temp', above=self._min_temp)
        self._pwm_cycle_time = config.getfloat(
            'pwm_cycle_time', .1, above=0., maxval=MAX_HEAT_TIME)
        self.max_power = config.getfloat('max_power', 1., above=0., maxval=1.)
        self.smooth_time = config.getfloat('smooth_time', 1., above=0.)

        # Setup sensor
        self._sensor = sensor
        self._sensor.setup_minmax(self._min_temp, self._max_temp)
        self._sensor.setup_callback(self.temperature_callback)

        # Setup temperature checks
        self.min_extrude_temp = config.getfloat(
            'min_extrude_temp', default=170.,
            minval=self._min_temp, maxval=self._max_temp)
        is_fileoutput = (self._printer.get_start_args().get('debugoutput')
                         is not None)
        ...
        self._smoothed_temp = 0.
        self.target_temp = 0.

        # Setup control algorithm sub-class
        algos = {'watermark': ControlBangBang, 'pid': ControlPID}
        algo = config.getchoice('control', algos)
        self._control = algo(self, config)
        ...
        self._printer.load_object(config, "verify_heater %s" % (self.short_name,))
        self._printer.load_object(config, "pid_calibrate")
        ...
        self._pwm_timer = self._reactor.register_timer(self._pwm_timer_callback)
    ...
    def set_pwm(self, read_time, value):
        if self.target_temp <= 0. or read_time > self.verify_mainthread_time:
            value = 0.
        if not self._last_pwm_time:
            self._last_pwm_time = self._last_temp_time
        sensor_report_time = self._sensor.get_report_time_delta()
        self._last_pwm_value = value
        self._last_pwm_time += max(sensor_report_time, self._pwm_cycle_time)
        self.mcu_pwm.set_pwm(self._last_pwm_time, value)

    def temperature_callback(self, read_time, temp):
        with self._lock:
            time_diff = read_time - self._last_temp_time
            self._last_temp = temp
            self._last_temp_time = read_time
            temp_diff = temp - self._smoothed_temp
            adj_time = min(time_diff * self.inv_smooth_time, 1.)
            self._smoothed_temp += temp_diff * adj_time
            self.can_extrude = (self._smoothed_temp >= self.min_extrude_temp)

    def _pwm_timer_callback(self, eventtime: float):
        sensor_report_time = self._sensor.get_report_time_delta()
        if self._last_temp_time != 0.:
            with self._lock:
                pwm_value = self._control.temperature_update(
                    self._last_temp_time, self._last_temp, self.target_temp)
                self.set_pwm(self._last_temp_time, pwm_value)
    ...
    def _handle_connect(self):
        self._reactor.update_timer(self._pwm_timer, self._reactor.NOW)
    ...
    def get_pwm_delay(self):
        return self._sensor.get_report_time_delta()
    def get_max_power(self):
        return self.max_power
    def get_smooth_time(self):
        return self.smooth_time
    def set_temp(self, degrees):
        if degrees and (degrees < self._min_temp or degrees > self._max_temp):
            raise self._printer.command_error(
                "Requested temperature (%.1f) out of range (%.1f:%.1f)"
                % (degrees, self._min_temp, self._max_temp))
        with self._lock:
    ...
    def get_temp(self, eventtime):
        ...
        with self._lock:
            if self._last_temp_time < quell_time:
                return 0., self.target_temp
            return self._smoothed_temp, self.target_temp
    def check_busy(self, eventtime):
        with self._lock:
            return self._control.check_busy(
                self._smoothed_temp, self.target_temp)
    ...
    def alter_target(self, target_temp):
        if target_temp:
            target_temp = max(self._min_temp, min(self._max_temp, target_temp))
    ...
    def stats(self, eventtime):
        ...
        with self._lock:
            ...
            last_pwm_value = self._last_pwm_value
    ...
    def get_status(self, eventtime):
        with self._lock:
            target_temp = self.target_temp
            smoothed_temp = self._smoothed_temp
            last_pwm_value = self._last_pwm_value
