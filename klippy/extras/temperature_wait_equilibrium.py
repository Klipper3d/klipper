...
class TemperatureWaitEquilibrium:
    def __init__(self, config):
        ...
        gcode.register_command("TEMPERATURE_WAIT_EQUILIBRIUM", self.cmd_TEMPERATURE_EQUILIBRIUM,
    ...
    def _tempRateLeastSquares(self, temp_values):
        ...
        # this is the slope of the line which is the rate of change of the temprature in degrees/second
    ...
    def cmd_TEMPERATURE_EQUILIBRIUM(self, gcmd):
        ...
        if sensor_name in heaters.heaters:
            raise gcmd.error("Sensor '%s' sensor is bound to a heater" % (sensor_name,))
        sensor = self.printer.lookup_object(sensor_name)
        # The default rate is 1 degree of change per minute, the minimum is 0.1
        target_rate_per_minute = gcmd.get_float('RATE', default = 1.0, minval = 0.1)
        min_data_points = 5      # To speed up "hot" print starts this algorithm only waits 5s before using the rate
        timespan = gcmd.get_float('TIMESPAN', default = 30, minval = min_data_points, maxval = 60)
        ...
        while not self.printer.is_shutdown():
            ...
            if len(temp_data) >= 2:
                rate_per_minute = round(self._tempRateLeastSquares(temp_data) * 60, 2)
                if len(temp_data) >= min_data_points and rate_per_minute <= target_rate_per_minute:
                    ...
                    return
                gcmd.respond_raw('{0}:{1}C @ {2}C/min /{3}C/min'.format(sensor_display_name, round(temp, 1), rate_per_minute, target_rate_per_minute))
