# SGP40 I2C-based VOC sensor support
#
...
class SGP40:
    def __init__(self, config):
        ...
        self.soft_reset_on_startup = config.getboolean(
            'soft_reset_on_startup', False)
    ...
    def _init_sgp40(self):
        ...
        if self.soft_reset_on_startup:
            ...
            self._write_command(SGP40_CMD["HEATER_OFF"], delay=.001)
    ...
    def _sample_sgp40(self, eventtime):
        # Read the measurement started last cycle. Some sensors need far longer
        # than the datasheet's 30ms, so we don't pause for it and use a full
        # report_time interval to wait for it.
        if self._measuring:
            self._measuring = False
            value = self._read(1)
        ...
        # Start the next measurement to read it at the start of next cycle
        try:
            ...
            logging.error(self._log_message(
                "Error starting measurement: {error}".format(error=e)))
    ...
    def _write_command(self, command, delay=0., read_len=0):
        try:
            ...
            logging.error(self._log_message(
                "I2C write failed: {error}".format(error=e)))
            return None
    ...
    def _read(self, read_len):
        ...
        if params is None:
            logging.error(self._log_message("I2C read failed after retries"))
            return None
        ...
        return [unpack_from(">H", response, i)[0] for i in range(0, reply_len,
                SGP40_WORD_LEN)]
...
# Class and algorithm to convert Sensirion SGP40 raw readings to VOC index.
# Ported from DFRobot's implementation:
# https://github.com/DFRobot/DFRobot_SGP40
# (Python/raspberrypi/DFRobot_SGP40_VOCAlgorithm.py)
# Copyright (c) 2010 DFRobot Co.Ltd (http://www.dfrobot.com)
# SPDX-License-Identifier: MIT
# Author(s): yangfeng
#
# Note: This class intentionally reimplements 16.16 fixed-point math
# (mul/div/sqrt/exp) instead of using Python's float math, to remain a faithful
# port of the original Sensirion/DFRobot reference algorithm and produce
# identical results.
class VOCAlgorithm:
    def __init__(self):
