...
class SNx4HC595(object):
    def __init__(self, config):
        ...
        ppins.setup_pin('digital_out', '%s:%s' % (self._mcu_name, self._data_pin))
        self._latch_pin = config.get('latch_pin')
        ppins.setup_pin('digital_out', '%s:%s' % (self._mcu_name, self._latch_pin))
        self._clock_pin = config.get('clock_pin')
        ppins.setup_pin('digital_out', '%s:%s' % (self._mcu_name, self._clock_pin))
        self._mcu.register_config_callback(self._build_config)
        self._oid = self._mcu.create_oid()
        self._mcu.register_response(self._update_host_bit_values, "hc595_bits", self._oid)
    ...
    def set_bit(self, bit, value):
        if (value == 0):
            self.write_bits(self.current_bits & ~(1 << bit))
        else:
    ...
    def _update_host_bit_values(self, params):
        ...
        #logging.info("hc595 value: %s \n" % self.current_bits)
    
    def add_cmd(self,cmd):
        self.cmds[self.last_update] = cmd
