...
class PRTouchCFG:
    def __init__(self, config):
        ...
        self.best_above_z       = config.getfloat('best_above_z', default=2, minval=0.5, maxval=10)      # above 
        ...
        self.max_press_cnt = config.getint('max_press_cnt', default=10, minval=2, maxval=20)
        self.extra_z_offset = config.getfloat('extra_z_offset', default = 0.0, minval=-2.0, maxval=10)
...
class PRTouchEndstopWrapper:
    def __init__(self, config):
        ...
        self.rdy_z_spd = config.getfloat('rdy_z_spd', default=10, minval=1, maxval=50)
    ...
    def _set_hot_temps(self, temp, fan_spd, wait=False, err=5):
        ...
        # self.obj.gcode.run_script_from_command('SET_PIN PIN=Part_Fan VALUE=%d' % (fan_spd))
        # self.obj.gcode.run_script_from_command('SET_PIN PIN=Side_Fan VALUE=%d' % (fan_spd))
        # self.obj.gcode.run_script_from_command('SET_PIN PIN=Exhaust_Fan VALUE=%d' % (fan_spd))
        if wait:
    ...
    def _probe_times(self, max_times, rdy_pos, speed_mm, min_dis_mm, max_z_err, min_hold, max_hold):
        if max_times > self.cfg.max_press_cnt:
            max_times = self.cfg.max_press_cntt
        o_mm = 0
        rdy_pos_z = rdy_pos[2]
    ...
    def shake_motor(self, cnt):
        ...
        for i in range(int(cnt / 2)):
            self.obj.gcode.run_script_from_command('G1 X%.2f Y%.2f Z%.2f F%d' % (now_pos[0] - self.cfg.shake_range, now_pos[1] - self.cfg.shake_range, now_pos[2] - self.cfg.shake_range / 2, int(60 * max_z_velocity * 0.8)))
            self.obj.gcode.run_script_from_command('G1 X%.2f Y%.2f Z%.2f F%d' % (now_pos[0] + self.cfg.shake_range, now_pos[1] - self.cfg.shake_range, now_pos[2] + self.cfg.shake_range / 2, int(60 * max_z_velocity * 0.8)))
            self.obj.gcode.run_script_from_command('G1 X%.2f Y%.2f Z%.2f F%d' % (now_pos[0] + self.cfg.shake_range, now_pos[1] + self.cfg.shake_range, now_pos[2] - self.cfg.shake_range / 2, int(60 * max_z_velocity * 0.8)))
            self.obj.gcode.run_script_from_command('G1 X%.2f Y%.2f Z%.2f F%d' % (now_pos[0] - self.cfg.shake_range, now_pos[1] + self.cfg.shake_range, now_pos[2] + self.cfg.shake_range / 2, int(60 * max_z_velocity * 0.8)))
            while len(self.obj.toolhead.move_queue.queue) >= 5:
                self.obj.hx711s.delay_s(0.010)
        self._move(now_pos, self.rdy_z_spd)
        pass
    ...
    def probe_ready(self):
        ...
        for i in range(4):
            self.val.rdy_pos[i][2] = self._probe_times(2, self.val.rdy_pos[i], self.cfg.g29_speed, 10, 0.2, self.cfg.min_hold, self.cfg.max_hold)
    ...
    def probe_z_offset(self):
        min_x, min_y = self.obj.bed_mesh.bmc.mesh_min
        max_x, max_y = self.obj.bed_mesh.bmc.mesh_max
        z_offset = self._probe_times(3, [max_x/2,max_y/2, self.cfg.bed_max_err + 1.], self.cfg.g29_speed, 10, self.cfg.check_bed_mesh_max_err, self.cfg.min_hold, self.cfg.max_hold)
        return z_offset
    ...
    def check_bed_mesh(self, auto_g29=True):
        ...
        for i in range(4):
            self.val.rdy_pos[i][2] = self._probe_times(2, self.val.rdy_pos[i], self.cfg.g29_speed, 10, self.cfg.check_bed_mesh_max_err * 2, self.cfg.min_hold, self.cfg.max_hold) + self.cfg.fix_z_offset
    ...
    def run_G29_Z(self):
        ...
        now_pos[2] = self._probe_times(2, now_pos, self.cfg.g29_speed, self.cfg.bed_max_err, 0.06, self.cfg.min_hold, self.cfg.max_hold) + self.get_best_rdy_z(now_pos[0], now_pos[1], self.val.gap_pos) + self.cfg.fix_z_offset
        ...
        if self.val.g29_cnt == x_cnt * y_cnt:
            ...
            home_z = self._probe_times(2, [self.val.home_xy[0], self.val.home_xy[1], self.cfg.bed_max_err + 1], 
    ...
    def cmd_MEASURE_GAP_TEST(self, gcmd):
        ...
        zero_z = self._probe_times(2, [self.val.rdy_pos[n_index[gap_index]][0], self.val.rdy_pos[n_index[gap_index]][1], self.cfg.bed_max_err + 1.], 
    ...
    def cmd_PRTOUCH_PROBE_ZOFFSET(self, gcmd):
        ...
        probe_x = (max_x - min_x) / 2 + min_x - probe_x_offset
        probe_y = (max_y - min_y) / 2 + min_y - probe_y_offset
        ...
        probe_gcmd = self.obj.gcode.create_gcode_command("PROBE", "PROBE", {'SAMPLES': '5'})

        z_probe = self.obj.probe.run_probe(probe_gcmd)
        self.pnt_msg("The length of the z_probe array is: %.3f" % len(z_probe))
        self.pnt_msg('Probe at sensor: %.3f' % z_probe[2])

        median = sorted(z_probe)[len(z_probe) // 2]
        self.pnt_msg('Median probe at: %.3f' % median)
        ...
        z_adjust = z_offset + start_z_offset + self.cfg.extra_z_offset
...
# g28_sta0_speed、g28_sta1_speed,g29_speed=1.8
