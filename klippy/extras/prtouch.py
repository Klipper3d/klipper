...
class PRTouchCFG:
    def __init__(self, config):
        ...
        self.clr_noz_wipe_speed = config.getfloat('clr_noz_wipe_speed', default=10, minval=10, maxval=50)
...
class PRTouchZOffsetWrapper:
    ...
    def clear_nozzle(self, hot_min_temp, hot_max_temp, bed_max_temp, min_hold, max_hold):
        ...
        self._move(end_pos[:2] + [end_pos[2] + self.cfg.pa_clr_down_mm], self.cfg.clr_noz_wipe_speed)
        self._set_hot_temps(temp=hot_min_temp, fan_spd=255, wait=True, err=5)
        self._move([end_pos[0], end_pos[1] + 10, end_pos[2] + 10], self.cfg.clr_noz_wipe_speed)
        self._set_hot_temps(temp=hot_min_temp, fan_spd=0, wait=False) 
