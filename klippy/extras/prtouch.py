...
class PRTouchConfig:
    ...
    def __init__(self, config):
        ...
        # FIX 1: pa_clr_dis_mm was read with getint (gi) but it is a millimetre
        # distance value used as a float minval constraint for clr_noz_len_y.
        # Using getfloat ensures consistent float arithmetic throughout.
        self.pa_clr_dis_mm  = gf('pa_clr_dis_mm',  default=5.0,  minval=2.0, maxval=100.0)
        self.pa_clr_down_mm = gf('pa_clr_down_mm', default=-0.1, minval=-1.0, maxval=1.0)
        ...
        self.clr_noz_len_y      = gf('clr_noz_len_y',      default=0,
                                      minval=self.pa_clr_dis_mm + 5.0, maxval=1000)
...
class PRTouchZOffsetWrapper:
    ...
    def _log_array(self, title: str, ary, tail: int = 32):
        ...
        if self.cfg.show_msg:
            # FIX 2: When len(ary) < tail, the original range(len(ary)-tail, len(ary))
            # produced negative start indices.  Python list indexing with negative values
            # wraps to the end of the list, so the wrong elements (or an IndexError for
            # indices smaller than -len(ary)) were displayed.  Clamping the start to 0
            # guarantees we always iterate over valid, non-wrapping indices.
            start = max(0, len(ary) - tail)
            body = ', '.join('%.2f' % ary[i] for i in range(start, len(ary)))
            self._log('%s [%s]' % (title, body))

    # Public aliases kept for external modules (filter, dirzctl, hx711s)
    # that call back into this object by the original names.
    def pnt_msg(self, msg: str):
        self._log(msg)

    def pnt_array(self, title: str, ary, tail: int = 32):
        self._log_array(title, ary, tail)
    ...
    def _check_trigger(self, ch: int, fit_vals: list, _unfit_vals: list,
        ...
        # FIX 3: When out_index == 0 the original loop started at i=0, so the
        # assignment `clean[0] = clean[0]*k + clean[-1]*(1-k)` used clean[-1]
        # (Python negative-index wrap), corrupting the first sample with the last.
        # Starting the smoothing pass at max(1, out_index) prevents the wraparound
        # while preserving the intended behaviour for out_index > 0.
        if self.state.out_index > 0:
            k = self.obj.filter.lft_k1_oft / 2.0
            for i in range(max(1, self.state.out_index), pi):
    ...
    def probe_by_step(self, rdy_pos: list, speed_mm: float, min_dis_mm: float,
        ...
        while self._system_ok():
            ...
            # The first return value of each cal_*_by_vals call (full filtered array)
            # is intentionally unused here; only the per-channel windowed slices
            # (tmp_unfit / tmp_fit) are needed for trigger detection.
            _, tmp_unfit = filt.cal_offset_by_vals(
                self.obj.hx711s.s_count, all_vals, filt.lft_k1_oft, self.cfg.pi_count)
            _, tmp_fit = filt.cal_filter_by_vals(
    ...
    def _on_trigger(self, ch: int, rdy_pos: list, step_us: int,
        ...
        filt = self.obj.filter
        # The first return value (full filtered array) is not needed here.
        _, tmp_cal = filt.cal_filter_by_vals(
    ...
    def cmd_PRTOUCH_PROBE_ZOFFSET(self, gcmd):
        ...
        # FIX 4: The original code set result[2] = -z_adjust.
        # _save_z_offset negates kin_pos[2], so it would have saved:
        #   -(-z_adjust) = z_adjust = start_z_offset - z_offset   ← WRONG
        # The value that must be persisted is z_offset (nozzle_z - reference_z).
        # Setting result[2] = -z_offset causes _save_z_offset to store:
        #   -(-z_offset) = z_offset                               ← CORRECT
        result    = list(z_probe)
        result[2] = -z_offset
