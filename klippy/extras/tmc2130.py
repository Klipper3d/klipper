...
class TMCCurrentHelper:
    def __init__(self, config, mcu_tmc):
        ...
        self.default_hold_current = config.getfloat('hold_current', self.default_run_current,
                                       above=0., maxval=MAX_CURRENT)
        self.homing_current = config.getfloat('homing_current', self.default_run_current,
                                       above=0., maxval=MAX_CURRENT)
        self.sense_resistor = config.getfloat('sense_resistor', 0.110, above=0.)
        vsense, irun, ihold = self._calc_current(self.default_run_current, self.default_hold_current)
        ...
        self.printer.register_event_handler("homing:homing_started", self._set_homing_current)
        self.printer.register_event_handler("homing:homing_finished", self._set_running_current)
        ...
        for rail in rails:
            for stepper in rail.get_steppers():
                ...
                if (stepper_name == self.name):
                    self.cmd_SET_TMC_CURRENT({"#original" : "SET_TMC_CURRENT STEPPER=%s CURRENT=%s" % (stepper_name, current), "CURRENT" : current})
        return True
    ...
    def cmd_SET_TMC_CURRENT(self, params):
        ...
        if 'HOLDCURRENT' not in params and 'CURRENT' not in params and 'HOMINGCURRENT' not in params:
            # Query only
            gcode.respond_info("Run Current: %0.2fA Hold Current: %0.2fA Homing Current: %0.2fA"
