...
class LDC1612:
    def __init__(self, config, calibration=None):
        ...
        self.freq = config.getint("freq", DEFAULT_LDC1612_FREQ, 2000000,
                                 40000000)
    ...
    def setup_home(self, print_time, trigger_freq,
        ...
        tfreq = int(trigger_freq * (1<<28) / float(self.freq) + 0.5)
    ...
    def _convert_samples(self, samples):
        freq_conv = float(self.freq) / (1<<28)
        count = 0
    ...
    def _start_measurements(self):
        ...
        rcount0 = self.freq / (16. * (self.data_rate - 4))
        self.set_reg(REG_RCOUNT0, int(rcount0 + 0.5))
        self.set_reg(REG_OFFSET0, 0)
        self.set_reg(REG_SETTLECOUNT0, int(SETTLETIME*self.freq/16. + .5))
