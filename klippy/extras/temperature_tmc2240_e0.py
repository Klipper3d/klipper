...
class PrinterTemperatureTMC2240_E0:
    def __init__(self, config):
        ...
        self.printer.add_object("temperature_tmc2240_e0 " + self.name, self)          
        self.sample_timer = self.reactor.register_timer(
            self._sample_tmc2240_e0_temperature)
    ...
    def _sample_tmc2240_e0_temperature(self, eventtime):
        self.tmc2240_temp = self._calc_temp_from_field()
        if self.temp < self.min_temp:
            self.printer.invoke_shutdown(
                "TMC2240_e0 temperature %0.1f below minimum temperature of %0.1f."
                % (self.temp, self.min_temp,))
        if self.temp > self.max_temp:
            self.printer.invoke_shutdown(
                "TMC2240_e0 temperature %0.1f above maximum temperature of %0.1f."
...
def load_config(config):
    ...
    pheaters.add_sensor_factory("temperature_tmc2240_e0", PrinterTemperatureTMC2240_E0)