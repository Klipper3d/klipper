...
import gcode, mathutil, stepper, configfile
    ...
    def __init__(self, config):
        ...
        if axis_name is None:
            if name in valid_axes:
                ...
                config.get('axis')  # Raise an error
        elif axis_name not in valid_axes:
            raise config.error("axis=%s is not valid in section '%s'"
                               % (axis_name, config.get_name()))
