...
class ToolHead:
    def __init__(self, config):
        ...
        gcode.register_command('SET_AXIS_LIMIT', self.cmd_SET_AXIS_LIMIT,
                                desc=self.cmd_SET_AXIS_LIMIT_help)  #####################################################################################      
    ...
    #########################################################################################################
    cmd_SET_AXIS_LIMIT_help = "Set printer velocity limits"##############################################
    def cmd_SET_AXIS_LIMIT(self, gcmd):                     #############################################
    ######################################################################################################### 
        x_min = gcmd.get_float('X_MIN', 1., minval=0.)
        x_max = gcmd.get_float('X_MAX', 4., minval=0.)
        y_min = gcmd.get_float('Y_MIN', 2., minval=0.)
        y_max = gcmd.get_float('Y_MAX', 5., minval=0.)
        z_min = gcmd.get_float('Z_MIN', 3., minval=0.)
        z_max = gcmd.get_float('Z_MAX', 6., minval=0.)
        
        self.kin.set_axes_limits(self.get_kinematics(), self.get_Coord())

    
        gcmd.respond_info('x_min={}, y_min={}, z_min={}'.format(x_min, y_min, z_min))
        #gcmd.respond_info('SET_AXIS_LIMIT: x_max=%.3, y_max=%.3, z_max%.3' % (x_max, y_max, z_max))
        gcmd.respond_info('reils: {}'.format(self.kin.set_axes_limits(self.get_kinematics(), self.get_Coord())))
        #gcmd.respond_info('axes: {}'.format(self.kin.axes_min))
