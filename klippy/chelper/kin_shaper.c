              ...
              , struct shaper_pulses *sp)
{
    ...
    for (i = sp->p_ind - 1; i >= 0; --i) {
        double t = move_time + sp->pulses[i].t;
        while (likely(t + offset < 0.)) {
            prev = list_prev_entry(prev, node);
            offset += prev->move_t;
        }
        res += sp->pulses[i].a * get_axis_position(prev, axis, t + offset);
    }
    offset = 0.;
    for (i = sp->p_ind; i < num_pulses; ++i) {
        double t = move_time + sp->pulses[i].t;
        while (likely(t - offset > m->move_t)) {
            offset += m->move_t;
            m = list_next_entry(m, node);
        }
        res += sp->pulses[i].a * get_axis_position(m, axis, t - offset);
    }
    return res;
}
