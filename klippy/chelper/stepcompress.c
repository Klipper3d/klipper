...
// Cancel/discard any not-yet-emitted pending steps without flushing them
// to the msg_queue. This is required during an asynchronous CANCEL_STEP
// flow: pending steps were generated by itersolve for the now-wiped trapq
// and would otherwise be emitted with stale clock intervals (relative to
// the OLD last_step_clock) that no longer match the MCU's freshly reset
// next_step_time, causing "Timer too close" shutdowns.
void __visible
