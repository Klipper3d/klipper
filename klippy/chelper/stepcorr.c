...
static inline double
calc_velocity_position_lag(struct stepper_corrections *sc, double velocity)
{
    double phase_lag = sc->velocity_lag_const * velocity;
    if (phase_lag > 1.)
        phase_lag = 1.;
                       ...
                       , double move_time)
{
    ...
    if ((!sc->stealthchop_lag_const && !sc->velocity_lag_const)
    ...
    return sk->calc_position_cb(sk, m, move_time)
        + calc_velocity_position_lag(sc, velocity);
}
                            ...
                            , double velocity_smooth_time)
{
    ...
    if (!sk->calc_smoothed_velocity_cb && (
                stealthchop_lag_const || velocity_lag_const))
        return -1;
    ...
    sc->velocity_lag_const = velocity_lag_const;
