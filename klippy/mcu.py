...
class MCU_pwm:
    ...
    def _build_config(self):
        ...
        self._set_cmd = self._mcu.lookup_command(
            "queue_digital_out oid=%c clock=%u on_ticks=%u", cq=cmd_queue)
        self._set_now_cmd = self._set_cmd
        hasty_cmd = "queue_digital_out_now oid=%c clock=%u on_ticks=%u"
        if self._mcu.try_lookup_command(hasty_cmd):
            self._set_now_cmd = self._mcu.lookup_command(hasty_cmd,
                                                         cq=cmd_queue)
