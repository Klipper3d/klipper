...
class GCodeParser:
    ...
    setGcodeOffsetM206_r = re.compile('^(?:[N][0-9]+)?\s*(SET_GCODE_OFFSET|M206)\s*[A-Z0-9=\s]*(Z|Z_ADJUST)[A-Z0-9=\s]*(?:\s|$)')
    def process_data(self, eventtime):
        ...
        for line in lines:
            # Execute imediatly if found M112
            if self.m112_r.match(line) is not None:
                self.cmd_M112({})
            # Execute imediatly if found SET_GCODE_OFFSET Z|Z_ADJUST or M206 Z
            elif self.setGcodeOffsetM206_r.match(line.upper()) is not None:
