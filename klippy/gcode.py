    ...
    def get(
        ...
        if value is None:
            if default is self.sentinel:
                raise self.error(
                    "Error on '{}': missing {}".format(self._commandline, name)
                )
            return default
        try:
            value = parser(value)
        ...
        if minval is not None and value < minval:
            raise self.error(
                "Error on '{}': {} must have minimum of {}".format(
                    self._commandline, name, minval
                )
            )
        if maxval is not None and value > maxval:
            raise self.error(
                "Error on '{}': {} must have maximum of {}".format(
                    self._commandline, name, maxval
                )
            )
        if above is not None and value <= above:
            raise self.error(
                "Error on '{}': {} must be above {}".format(
                    self._commandline, name, above
                )
            )
        if below is not None and value >= below:
            raise self.error(
                "Error on '{}': {} must be below {}".format(
...
class GCodeDispatch:
    ...
    @classmethod
    def is_traditional_gcode(cls, cmd):
        """Return True if cmd is a traditional g-code.

        Args:
            cmd (str): The g-code command.

        Returns:
            bool: True if the given cmd is a traditional g-code command, False
                otherwise.
        """
    ...
    def register_command(self, cmd, func, when_not_ready=False, desc=None):
        ...
        if cmd in self.ready_gcode_handlers:
            raise self.printer.config_error(
                "gcode command {} already registered".format(cmd)
            )
    ...
    def register_mux_command(self, cmd, key, value, func, desc=None):
        ...
        if prev_key != key:
            raise self.printer.config_error(
                "mux command {} {} {} may have only one key ({})".format(
                    cmd, key, value, prev_key
                )
            )
        if value in prev_values:
            raise self.printer.config_error(
                "mux command {} {} {} already registered ({})".format(
    ...
    def _process_commands(self, commands, need_ack=True):
        for line in commands:
            ...
            try:
                ...
                msg = 'Internal error on command:"{}"'.format(cmd)
    ...
    def _respond_error(self, msg):
        ...
        self.respond_raw("!! {}".format(lines[0].strip()))
        if self.is_fileinput:
            self.printer.request_exit("error_exit")

    def _respond_state(self, state):
        self.respond_info("Klipper state: {}".format(state), log=False)
    ...
    def cmd_default(self, gcmd):
        ...
        if cmd.startswith("M117 ") or cmd.startswith("M118 "):
            ...
            return
        gcmd.respond_info('Unknown command:"{}"'.format(cmd))
    ...
    def _cmd_mux(self, command, gcmd):
        ...
        if key_param not in values:
            raise gcmd.error(
                "The value '{}' is not valid for {}".format(key_param, key)
            )
    ...
    def cmd_M115(self, gcmd):
        ...
        msg = " ".join(["{}:{}".format(k, v) for k, v in kw.items()])
    ...
    def cmd_HELP(self, gcmd):
        ...
        for cmd in sorted(self.gcode_handlers):
            if cmd in self.gcode_help:
                cmdhelp.append("{:10s}: {}".format(cmd, self.gcode_help[cmd]))
...
class GCodeIO:
    ...
    def _dump_debug(self):
        ...
        for eventtime, data in self.input_log:
            out.append("Read {:f}: {}".format(eventtime, repr(data)))
        logging.info("\n".join(out))
