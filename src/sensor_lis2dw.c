    ...
    switch (args[2]) {
        case SPI_SERIAL:
            if (CONFIG_HAVE_GPIO_SPI) {
                ax->bus = spidev_oid_lookup(args[1]);
        ...
        case I2C_SERIAL:
            if (CONFIG_HAVE_GPIO_I2C) {
               ax->bus = i2cdev_oid_lookup(args[1]);
...
lis2dw_query(struct lis2dw *ax, uint8_t oid)
{
    ...
    if (CONFIG_HAVE_GPIO_SPI && ax->bus_type == SPI_SERIAL) {
        ...
        spidev_transfer(ax->bus, 1, sizeof(msg), msg);

        spidev_transfer(ax->bus, 1, sizeof(fifo), fifo);
        ...
        struct i2cdev_s *i2c = ax->bus;

        i2c_read(i2c->i2c_config, sizeof(msg_reg), msg_reg
                    , sizeof(msg), msg);

        i2c_read(i2c->i2c_config, sizeof(fifo_reg), fifo_reg
                    , sizeof(fifo), fifo);
