...
void pio_sm_claim(PIO pio, uint sm) {
    ...
    if (which) {
        hw_claim_or_assert(&claimed, NUM_PIO_STATE_MACHINES + sm
...
static int _pio_find_offset_for_program(PIO pio
    ...
    if (program->origin >= 0) {
        ...
        return used_mask & (program_mask << program->origin) ? -1
            : program->origin;
    } else {
...
static bool _pio_can_add_program_at_offset(PIO pio
...
static void _pio_add_program_at_offset(PIO pio, const pio_program_t *program
    , uint offset) {
    ...
    for (uint i = 0; i < program->length; ++i) {
        ...
        pio->instr_mem[offset + i] = pio_instr_bits_jmp
            != _pio_major_instr_bits(instr) ? instr : instr + offset;
    }
    uint32_t program_mask = (1u << program->length) - 1;
...
void pio_add_program_at_offset(PIO pio, const pio_program_t *program
    , uint offset) {
    uint32_t save = hw_claim_lock();
...
void pio_remove_program(PIO pio, const pio_program_t *program
    ...
    uint32_t save = hw_claim_lock();
    assert(program_mask == (_used_instruction_space[pio_get_index(pio)]
        & program_mask));
...
void pio_sm_set_pins_with_mask(PIO pio, uint sm, uint32_t pinvals
    , uint32_t pin_mask) {
    check_pio_param(pio);
    check_sm_param(sm);
    uint32_t pinctrl_saved = pio->sm[sm].pinctrl;
    while (pin_mask) {
        ...
        pio->sm[sm].pinctrl =
                ...
                (base << PIO_SM0_PINCTRL_SET_BASE_LSB);
        pio_sm_exec(pio, sm, pio_encode_set(pio_pins
            , (pinvals >> base) & 0x1u));
        pin_mask &= pin_mask - 1;
    }
    pio->sm[sm].pinctrl = pinctrl_saved;
...
void pio_sm_set_pindirs_with_mask(PIO pio, uint sm, uint32_t pindirs
    , uint32_t pin_mask) {
    check_pio_param(pio);
    check_sm_param(sm);
    uint32_t pinctrl_saved = pio->sm[sm].pinctrl;
    while (pin_mask) {
        ...
        pio->sm[sm].pinctrl =
                (1u << PIO_SM0_PINCTRL_SET_COUNT_LSB) |
                (base << PIO_SM0_PINCTRL_SET_BASE_LSB);
        pio_sm_exec(pio, sm, pio_encode_set(pio_pindirs
            , (pindirs >> base) & 0x1u));
        pin_mask &= pin_mask - 1;
    }
    pio->sm[sm].pinctrl = pinctrl_saved;
...
void pio_sm_drain_tx_fifo(PIO pio, uint sm) {
    uint instr = (pio->sm[sm].shiftctrl
        & PIO_SM0_SHIFTCTRL_AUTOPULL_BITS) ? pio_encode_out(pio_null, 32) :
