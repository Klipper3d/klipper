...
void clocks_init(void) {
...
#if false
    // Everything is 48MHz on FPGA apart from RTC. Otherwise set to
    // 0 and will be set in clock configure
    if (running_on_fpga()) {
    ...
    // Before we touch PLLs, switch sys and ref cleanly away from
    // their aux sources.
...
static void clocks_handle_resus(void) {
    ...
    hw_set_bits(&clocks_hw->resus.ctrl
...
bool clock_configure_gpin(enum clock_index clk_index
    , uint gpio, uint32_t src_freq, uint32_t freq) {
