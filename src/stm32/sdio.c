...
// PINS: CLK -> PC12 , CMD -> PD2, DAT0 -> PC8, DAT1 -> PC9, DAT2 -> PC10, DAT3 -> PC11
...
static const struct sdio_info sdio_bus[] = {
    { SDIO, GPIO('C', 12), GPIO('D', 2), GPIO('C', 8), GPIO('C', 9), GPIO('C', 10), GPIO('C', 11), SDIO_FUNCTION },
};

#define SDIO_MAX_TIMEOUT 500 // Wait for at least 500ms before a timeout occurs
#define CLKCR_CLEAR_MASK (SDIO_CLKCR_CLKDIV | SDIO_CLKCR_PWRSAV | SDIO_CLKCR_BYPASS | SDIO_CLKCR_WIDBUS | SDIO_CLKCR_NEGEDGE | SDIO_CLKCR_HWFC_EN)
#define DCTRL_CLEAR_MASK (SDIO_DCTRL_DTEN | SDIO_DCTRL_DTDIR | SDIO_DCTRL_DTMODE | SDIO_DCTRL_DBLOCKSIZE)
#define CMD_CLEAR_MASK (SDIO_CMD_CMDINDEX | SDIO_CMD_WAITRESP | SDIO_CMD_WAITINT | SDIO_CMD_WAITPEND | SDIO_CMD_CPSMEN | SDIO_CMD_SDIOSUSPEND)
...
#define SDIO_CMD_FLAGS (SDIO_STA_CCRCFAIL | SDIO_STA_CTIMEOUT | SDIO_STA_CMDREND | SDIO_STA_CMDSENT)
#define SDIO_STATIC_FLAGS (SDIO_STA_CCRCFAIL | SDIO_STA_DCRCFAIL | SDIO_STA_CTIMEOUT | SDIO_STA_DTIMEOUT | SDIO_STA_TXUNDERR | SDIO_STA_RXOVERR | SDIO_STA_CMDREND | SDIO_STA_CMDSENT | SDIO_STA_DATAEND | SDIO_STA_DBCKEND | SDIO_STA_SDIOIT)
...
sdio_setup(uint32_t bus)
{
    ...
    if (!is_enabled_pclock((uint32_t)sdio)) {
        ...
        // Initialize pins
        gpio_peripheral_speed(sdio_bus[bus].dat0_pin, sdio_bus[bus].function, 1, 0x03);
        gpio_peripheral_speed(sdio_bus[bus].dat1_pin, sdio_bus[bus].function, 1, 0x03);
        gpio_peripheral_speed(sdio_bus[bus].dat2_pin, sdio_bus[bus].function, 1, 0x03);
        gpio_peripheral_speed(sdio_bus[bus].dat3_pin, sdio_bus[bus].function, 1, 0x03);
        gpio_peripheral_speed(sdio_bus[bus].cmd_pin, sdio_bus[bus].function, 1, 0x03);
        gpio_peripheral_speed(sdio_bus[bus].clk_pin, sdio_bus[bus].function, 1, 0x03);
    }

    // Setup SDIO with 1 bit width first and slow clock <400 kHz
    uint32_t sdio_confreg = SDIO_CLOCK_EDGE_RISING | SDIO_CLOCK_BYPASS_DISABLE | SDIO_CLOCK_POWER_SAVE_DISABLE | SDIO_BUS_WIDE_1B | SDIO_HARDWARE_FLOW_CONTROL_DISABLE | SDIO_INIT_CLK_DIV;
...
uint8_t
sdio_send_command(struct sdio_config sdio_config, uint8_t cmd, uint32_t argument, uint8_t wait, uint8_t *response_data, uint8_t *response_data_len) 
{
    SDIO_TypeDef *sdio = sdio_config.sdio;
    uint32_t wait_flags = 0; // valid for SDIO_WAIT_NO_RESPONSE
    uint32_t sta_flags = (wait == SDIO_WAIT_NO_RESPONSE) ? SDIO_STA_CMDSENT : SDIO_STA_CCRCFAIL | SDIO_STA_CMDREND | SDIO_STA_CTIMEOUT;
    ...
    // CMD and State Machine enabled. Wait for response like specified by wait_flags.
    ...
    // Step 3: Store response_data and check for short and long responses timeout and crc
...
sdio_get_dctrl_blocksize(uint32_t value)
{
    switch(value) {
        ...
        case 128:   return SDIO_DCTRL_DBLOCKSIZE_2|SDIO_DCTRL_DBLOCKSIZE_1|SDIO_DCTRL_DBLOCKSIZE_0;
        ...
        case 2048:  return SDIO_DCTRL_DBLOCKSIZE_3|SDIO_DCTRL_DBLOCKSIZE_1|SDIO_DCTRL_DBLOCKSIZE_0;
        case 4096:  return SDIO_DCTRL_DBLOCKSIZE_3|SDIO_DCTRL_DBLOCKSIZE_2;
        case 8192:  return SDIO_DCTRL_DBLOCKSIZE_3|SDIO_DCTRL_DBLOCKSIZE_2|SDIO_DCTRL_DBLOCKSIZE_0;
        case 16384: return SDIO_DCTRL_DBLOCKSIZE_3|SDIO_DCTRL_DBLOCKSIZE_2|SDIO_DCTRL_DBLOCKSIZE_1;
    }
    return SDIO_DCTRL_DBLOCKSIZE_Msk;
}

uint8_t
sdio_prepare_data_transfer(struct sdio_config sdio_config, uint8_t read, uint32_t numblocks, uint32_t blocksize)
{
    SDIO_TypeDef *sdio = sdio_config.sdio;
    uint32_t dctrl_blocksize = sdio_get_dctrl_blocksize(blocksize);
    uint32_t reg = dctrl_blocksize | ((read > 0) ? SDIO_DCTRL_DTDIR : 0U) | SDIO_DCTRL_DTEN;
...
uint8_t
sdio_read_data(struct sdio_config sdio_config, uint8_t *data, uint32_t numblocks, uint32_t blocksize)
{
    ...
    while ((sdio->STA & (SDIO_STA_RXOVERR | SDIO_STA_DCRCFAIL | SDIO_STA_DTIMEOUT | SDIO_STA_DATAEND)) == 0) {
...
uint8_t
sdio_write_data(struct sdio_config sdio_config, uint8_t *data, uint32_t numblocks, uint32_t blocksize)
{
    ...
    while ((sdio->STA & (SDIO_STA_TXUNDERR | SDIO_STA_DCRCFAIL | SDIO_STA_DTIMEOUT | SDIO_STA_DATAEND)) == 0) {
...
void
sdio_send_cmd(struct sdio_config sdio_config, uint8_t cmd, uint32_t argument, uint8_t wait)
{
    SDIO_TypeDef *sdio = sdio_config.sdio;
...
void
sdio_switch_bus_width_and_speed(struct sdio_config sdio_config, uint8_t width, uint8_t clkdiv)
{
    SDIO_TypeDef *sdio = sdio_config.sdio;

    if ((width == 1) || (width == 4)) {
        uint32_t sdio_confreg = SDIO_CLOCK_EDGE_RISING | SDIO_CLOCK_BYPASS_DISABLE | SDIO_CLOCK_POWER_SAVE_DISABLE | ((width == 4) ? SDIO_BUS_WIDE_4B : SDIO_BUS_WIDE_1B) | SDIO_HARDWARE_FLOW_CONTROL_ENABLE | (clkdiv & 0xFF);
        MODIFY_REG(sdio->CLKCR, CLKCR_CLEAR_MASK, sdio_confreg);
    }
}
