...
#include "board/serial_bridge_irq.h" // serial_rx_data
...
UART0_IRQHandler(void)
{
    ...
    if (status == 0x04)
        serial_bridge_rx_byte(LPC_UART0->RBR);
    else if (status == 0x02)
        kick_tx();
}
...
DECL_CONSTANT_STR("RESERVE_PINS_serial_bridge", "P0.3,P0.2");
...
serial_bridge_init(void)
{
    ...
    uint32_t div = pclk / (CONFIG_SERIAL_BRIDGE_BAUD * 16);
