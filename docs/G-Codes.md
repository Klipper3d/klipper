...
`INITIALIZE_TOOLCHANGER [TOOLCHANGER=toolchanger] [TOOL=<name>]`: Initializes or
Re-initializes the toolchanger state. Sets toolchanger status to 'OK'.

The default behavior is to auto-initialize on first tool selection call.
This can be overriden by setting `toolchanger.auto_initialize: false`.
Always needs to be manually re-initialized after a `SELECT_TOOL_ERROR`. 
If `TOOL` is specified, sets the active tool without performing any tool change
gcode. The after_change_gcode is always called. `TOOL` with empty name unselects
tool.
...
or set `tool.register_macro: true` to auto register a T macro.
...
### UNSELECT_TOOL
`UNSELECT_TOOL`: Unselect active tool without selecting a new one.
